...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log('Microsoft Calendar sync action:', action);

    // Get the user's Microsoft Calendar connection
    ...
    if (!connection) {
      return new Response(
        JSON.stringify({ error: 'No Microsoft Calendar connection found. Please connect your Microsoft account first.' }),
    ...
    if (new Date(connection.token_expiry) <= new Date()) {
      ...
      return new Response(
        JSON.stringify({ error: 'Token expired. Please reconnect your Microsoft account.' }),
    ...
    if (action === 'fetch') {
      ...
      return new Response(
        JSON.stringify({ 
          success: true, 
          message: `Synced ${msEvents.length} events from Microsoft Calendar`,
    ...
    if (action === 'export') {
      // Export events to Microsoft Calendar
      ...
      return new Response(
        JSON.stringify({ 
          success: true, 
          message: `Exported ${exportedCount} events to Microsoft Calendar`,
          count: exportedCount
        }),
        { headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
